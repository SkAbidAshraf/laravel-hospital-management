...
@section('content')
    ...
    <div class="card my-4 shadow border-0">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('dashboard.services.add.submit') }}" enctype="multipart/form-data">
                @csrf

                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <div class="my-4">
                    <label>Add Service Header</label>
                    <input type="text" name="title" class="form-control @error('title') is-invalid @enderror"
                        value="{{ old('title') }}" placeholder="Service Header">
                    @error('title')
                ...
                <div class="mt-4">
                    ...
                    <textarea class="form-control" id="ck_editor" row="10" name="details">{{ old('details') }}</textarea>
                </div>
                @error('details')
